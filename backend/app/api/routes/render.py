...
async def queue_render(project_id: UUID, db: AsyncSession = Depends(get_db)) -> dict[str, str]:
    ...
    job = RenderJob(project_id=project_id, celery_task_id=task.id)
    db.add(job)
    await db.commit()
...
@router.get("/projects/{project_id}/status", response_model=RenderJobOut)
async def get_status(project_id: UUID, db: AsyncSession = Depends(get_db)) -> RenderJob:
