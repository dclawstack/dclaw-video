...
async def _update_job_progress(job_id: str, percent: int, logs: str = "") -> None:
    async with _get_db() as db:
        result = await db.execute(
            select(RenderJob).where(RenderJob.celery_task_id == job_id)
        )
...
def generate_storyboard(self: Any, project_id: str) -> dict[str, Any]:
    async def _run() -> dict[str, Any]:
        await _update_job_progress(
            self.request.id, 0, "Starting storyboard generation..."
        )
        async with _get_db() as db:
            result = await db.execute(
                select(Scene)
                .where(Scene.project_id == UUID(project_id))
            ...
            for idx, scene in enumerate(scenes):
                ...
                await _update_job_progress(
                    self.request.id, pct, f"Scene {idx + 1}/{total} storyboard done"
                )
...
def render_scene(self: Any, scene_id: str) -> dict[str, Any]:
    async def _run() -> dict[str, Any]:
        await _update_job_progress(
            self.request.id, 0, f"Starting scene render {scene_id}"
        )
        async with _get_db() as db:
            result = await db.execute(
                select(Scene).where(Scene.id == UUID(scene_id))
            )
            ...
            tts = TTSService()
            tts.synthesize(scene.narration_text)

            comfy = ComfyUIClient()
            workflow = comfy.build_animatediff_workflow(
                scene.visual_prompt or scene.narration_text
            )
            prompt_id = await comfy.queue_prompt(workflow)
        ...
        await _update_job_progress(
            self.request.id, 100, f"Scene {scene_id} rendered"
        )
...
def render_project(self: Any, project_id: str) -> dict[str, Any]:
    async def _run() -> dict[str, Any]:
        await _update_job_progress(
            self.request.id, 0, "Starting full project render..."
        )
        async with _get_db() as db:
            result = await db.execute(
                select(Scene)
            ...
            for idx, scene in enumerate(scenes):
                if scene.status != "done" or not scene.video_clip_url:
                    ...
                    del audio_path  # TODO: use audio in assembly
                ...
                video_paths.append(
                    assembler.write_video(str(scene.id), b"")
                )
                pct = int((idx + 1) / total * 50)
                await _update_job_progress(
                    self.request.id, pct, f"Scene {idx + 1}/{total} ready"
                )
            ...
            if project:
                project.status = "done"
                project.video_url = (
                    f"minio://{settings.minio_bucket}/renders/{project_id}.mp4"
                )
                await db.commit()

        await _update_job_progress(
            self.request.id, 100, "Render complete"
        )
        return {
